...
int main() {
  NetworkController controller = build_network(9, 100, 59, 4, 88, 3);
...
//   fptr = fopen("output.csv","w");
  ...
  for (int i_its = 0; i_its < n_its; i_its++) {
    for (int i_seq = 0; i_seq < input_seq_length; i_seq++) {
        ...
        // printf("Input: %f, %f, %f, %f, %f, %f\n", controller.hid2_in[0], controller.hid2_in[1], controller.hid2_in[2], controller.hid2_in[3], controller.hid2_in[4], controller.hid2_in[5]);
        // Forward network
        forward_network(&controller);
        printf("Output: %f, %f, %f, %f, %f\n", controller.out[0], controller.out[1], controller.out[3], controller.integ_out[0], controller.integ_out[1]);
    }
    reset_network(&controller);
  }
  end_t = clock();
